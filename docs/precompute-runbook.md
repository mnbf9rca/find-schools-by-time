...
2. In `motis-spike/data.rail/config.yml`, set `limits.onetomany_max_many` to 20000, all the batches need, and `routing_max_timeout_seconds` to 600. Set `max_max_matching_distance` to 1000 for the fallback; main requests still match at 250 metres.
...
5. Run the 100 school sample end to end, issue #14's done condition: `uv run --with pyproj fixtures/precompute.py motis-spike/sample --schools fixtures/spike-sample.csv --workers 12`. Check its walk coverage first: an origin within 2 kilometres of a school with no walking value is a defect to chase now, not after the full run.
6. Start the full run in a third stoppable task: `uv run --with pyproj fixtures/precompute.py motis-spike/precompute --workers 12`.
...
Record two numbers against the prediction, in `motis-spike/NOTES.md` and in a comment on issue #17. Wall time: expect about 3 hours 15 minutes at twelve workers, projected from run 3's 4 hours 8 minutes at eight workers. The 100-school sweep measured 311 seconds at eight workers, 243 at twelve and 222 at sixteen, excluding fallback and transpose. All school files were byte-identical, with flat peak server memory of about 9.2 GiB across the three sweeps. Time the transpose separately. Peak server resident set size: expect about 12 GB for the full refresh, based on run 3. Expect about 3.8 GB of disk, the school files plus the records.
...
Run 3, version 20260915T011918Z, added the 1,800 second access walk and the fallback pass: 4 hours 8 minutes of wall time at eight workers and a peak server resident set of 11.77 GB, with the fallback covering 887 origins in 2,661 requests.
