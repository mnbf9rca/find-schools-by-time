...
2. In `motis-spike/data.rail/config.yml`, set `limits.onetomany_max_many` to at least 20000, `routing_max_timeout_seconds` to 600, and `max_max_matching_distance` to 500. The three main requests still use 250 metres; only the forward walking gap-fill uses 500.
...
8. After an interruption, restart the server if it stopped, restart `caffeinate` with the new process identifier, then rerun step 6. Completed schools are skipped. Runs created before the walking gap-fill must use a fresh output directory because their resume parameters differ; the school binary layout is unchanged.
