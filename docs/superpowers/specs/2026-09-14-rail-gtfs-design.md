...
Before `library(UK2GTFS)` it sets `options(UK2GTFS_opt_updateCachedDataOnLibaryLoad = FALSE)`, so a load cannot swap the cached data. It records `packageDescription("UK2GTFS")$RemoteSha`, the UK2GTFS-data release commit verified by matching its bundled table checksums, and checksums of the cached tiploc and agency tables. UK2GTFS-data is a data repository, not an R package. A version will not do: UK2GTFS has no tag matching the reported 0.4.0 (tags run 0.001 to 0.003). Both SHAs go in `NOTES.md` and the licence rows.
