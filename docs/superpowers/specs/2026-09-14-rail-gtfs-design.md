...
2. Check every rail stop sits between latitude 49.8 and 61.0 and longitude -8.7 and 1.9, and count stops with no coordinates. Check the six spot-check stations land within 500 m of their known positions, and list tiplocs whose coordinates came from the MSN fallback. Assert that the only stop outside these bounds is HOEKVHL, the real overseas ferry terminal at Hoek van Holland.
