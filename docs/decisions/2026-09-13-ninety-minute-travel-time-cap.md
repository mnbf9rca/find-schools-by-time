...
The transit numbers the sweep stores are not yet journey durations. Under `arriveBy`, MOTIS returns 08:30 minus the departure time of the latest feasible journey, which includes the slack before the deadline, so transit values run 10 to 56 minutes above the actual journey and are not comparable with the street modes. Issue #39 settles which metric the precompute stores. The 90 minute cap is expressed in whichever metric that issue lands on, and the cost measurements above will need rechecking if the metric changes what falls inside the cap.
