    ...
    def test_fallback_fills_only_empty_origins_and_resumes_without_requests(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                for b in outward:
                    self.assertEqual(b['maxMatchingDistance'], 1000)
                    if 'mode' in b:
                ...
                self.assertEqual(manifest.get('fallback_matching_metres'), 1000)
                ...
                state['parameters']['fallback_version'] = 1
                (out / 'run.json').write_text(json.dumps(state))
                result = subprocess.run(command, capture_output=True, text=True, timeout=30)
                self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
                self.assertEqual(len(calls), 6)
                self.assertTrue(all(not b['arriveBy'] and b['maxMatchingDistance'] == 1000 for b in calls))
                self.assertEqual({p.name: p.read_bytes() for p in (out / 'schools').glob('*.bin')}, saved)
                self.assertEqual((out / version / '530_180.bin').read_bytes(), empty)
                state = json.loads((out / 'run.json').read_text())
                self.assertEqual(state['parameters']['fallback_version'], 2)
                self.assertEqual(state['fallback_requests'], 12)
                calls.clear()
                result = subprocess.run(command, capture_output=True, text=True, timeout=30)
                self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
                self.assertEqual(calls, [])
